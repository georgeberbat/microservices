    ...
    public static class SerilogRequestLoggerHelper
    {
        ...
        private static readonly MapperConfiguration MapperConfiguration = new(cfg =>
        {
            cfg.CreateMap<HttpRequest, HttpRequestInfo>()
                ...
                .ForMember(info => info.RequestUrl,
                    expression => expression.MapFrom(request => request.GetDisplayUrl()));
        });
        ...
        public static void SetupSerilogEnrichAction(RequestLoggingOptions options)
        {
            ...
            options.EnrichDiagnosticContext = (diagnosticsContext, httpContext) =>
            {
                ...
                if (request.Headers.Count > 0) MaskValues("headers_", request.Headers);

                if (request.Cookies.Count > 0) MaskValues("cookies_", request.Cookies);

                if (request.Query.Count > 0) MaskValues("queries_", request.Query);

                if (request.HasFormContentType && request.Form.Count > 0) MaskValues("forms_", request.Form);
        ...
        {
            var request = httpContext.Request;
            if (request.ContentType?.Equals(HeaderApplicationGrpc, StringComparison.OrdinalIgnoreCase) == true)
                return null;

            return GetMapper().Map<HttpRequestInfo>(request);
        }
